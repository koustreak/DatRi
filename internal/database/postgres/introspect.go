...
// NewIntrospector creates a new PostgreSQL schema introspector
...
func (p *Introspector) TableExists(ctx context.Context, schema, table string) (bool, error) {
	const q = `
		SELECT EXISTS (
...
func (p *Introspector) InspectTable(ctx context.Context, schema, table string) (*database.TableInfo, error) {
	const q = `
		SELECT
			c.column_name,
			c.data_type,
...
func (p *Introspector) ListForeignKeys(ctx context.Context, schema string) ([]database.ForeignKey, error) {
	const q = `
		SELECT
			tc.constraint_name,
			kcu.table_name,
